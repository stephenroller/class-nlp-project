...
Ciaramita and Johnson also impose these same restrictions on their test set \cite{cj} since these restrictions allow one to ignore the word sense disambiguation problem entirely. While Curran uses the same test set as Ciaramita and Johnson \cite{curran}, he does not require his words appear within the corpus due to his use of a morphological backoff model.
...
Note the values collected in Figure \ref{fig:res} use $n=9$, which is below the optimal value on the curve in Figure \ref{fig:n}, but more computationally efficient.

INSERT GRAPH SHOWING EFFECT OF MODIFYING $k$ HERE
