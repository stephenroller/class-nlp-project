...
We did not expect to perform as well as \cite{curran} on the task, as our model has significantly fewer features, uses a smaller data set, and has no backoff model. Furthermore, this was not the precise goal of our work. Rather, we aimed to test the influence of search engine results and use this task as a proxy.

We hypothesized that greater degrees of web influence would correlate with higher accuracies and F-scores. That is, we expected the use of web scraping to outperform the use of search engine descriptions, which would outperform the use of no engine results at all.
...
\begin{figure}[hbtp]
\begin{center}
\includegraphics[width=120mm]{graphs/damping.png}
\end{center}
\caption{Effect of damping coefficient on F-score}
\label{fig:damping}
\end{figure}
We use a development set of 100 held-out words from WordNet 1.6 to tune the parameters relating to voting.
Figure \ref{fig:damping} gives the performance of the algorithm with different damping coefficient values using scraping for initial enrichment and descriptions for refinement.

MORE VOTING GRAPHS ALSO GO HERE
